...
import { Connection, ConnectionProperty, Contract, Client, Tenant, Tag, Datasource, datasource, WorkflowDesign, Workflow, WorkflowHelper, ActionConfiguration, DatasourceHelper, datasourcePayload } from "@ntx-xt/nac-client";
...
const tags: ExecutionCallback<Tag[]> = async (client: Client): Promise<Tag[]> => {
    return await client.getTags()
}

const contracts: ExecutionCallback<Contract[]> = async (client: Client): Promise<Contract[]> => {
    return await client.getContracts()
}

const connections: ExecutionCallback<Connection[]> = async (client: Client): Promise<Connection[]> => {
    return await client.getConnections()
}

const connection: ExecutionCallback<Connection | undefined> = async (client: Client, request: HttpRequest, context: Context): Promise<Connection | undefined> => {
...
const datasources: ExecutionCallback<Datasource[]> = async (client: Client, request: HttpRequest): Promise<Datasource[]> => {
...
const datasource: ExecutionCallback<Datasource> = async (client: Client, request: HttpRequest): Promise<Datasource | undefined> => {
    if (request.method === "GET") {
        return request.query.name
...
const contract: ExecutionCallback<Contract | undefined> = async (client: Client, request: HttpRequest): Promise<Contract | undefined> => {
...
const workflow: ExecutionCallback<XtensionWorkflow> = async (client: Client, request: HttpRequest): Promise<XtensionWorkflow> => {
    if (request.query.id) {
...
const exportWorkflow: ExecutionCallback<string> = async (client: Client, request: HttpRequest): Promise<string> => {
    return await client.exportWorkflow(request.params.workflowId)
}

const importWorkflow: ExecutionCallback<XtensionWorkflow> = async (client: Client, request: HttpRequest): Promise<XtensionWorkflow> =>
    _toXtensionWorkflow(
        await client.importWorkflow(request.body.name, request.body.key, request.body.overwrite))

const changeConnection: ExecutionCallback<XtensionWorkflow> = async (client: Client, request: HttpRequest): Promise<XtensionWorkflow> => {
...
const changeDatasource: ExecutionCallback<XtensionWorkflow> = async (client: Client, request: HttpRequest): Promise<XtensionWorkflow> => {
    const workflow = await client.getWorkflow(request.params.workflowId)
...
const publish: ExecutionCallback<XtensionWorkflow> = async (client: Client, request: HttpRequest): Promise<XtensionWorkflow> => {
    const workflow = await client.getWorkflow(request.params.workflowId)
...
export type ExecutionCallback<ReturnType = any> = (client: Client, request: HttpRequest, context: Context) => Promise<ReturnType | void>;
